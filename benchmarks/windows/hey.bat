...
hey -n %REQUESTS% -c %CONCURRENCY% -m POST -H "Content-Type: application/json" -d "{\"name\":\"LoadUser\",\"email\":\"load@example.com\",\"password\":\"loadtest123\"}" "%BASE_URL%/api/v1/users"
...
hey -n %REQUESTS% -c %CONCURRENCY% -m PUT -H "Content-Type: application/json" -d "{\"name\":\"Updated\",\"email\":\"updated@example.com\",\"password\":\"newpass123\"}" "%BASE_URL%/api/v1/users/1"
